...
function areThereAnyCookies(){
    ...
    if (cookiesAccepte == true || cookiesAccepte == 'true') {
        if (getCookie('wallet') == null) {
            ...
            console.log('le porte feuille est :' + getCookie('wallet'));
        }else if (getCookie('case') == null) {
            console.log('wallet is empty');
            document.cookie = `case=${actualCase}; expires=expires=${date};`;
            console.log('le porte feuille est :' + getCookie('case'));
        }
    }
}
...
function partie(){
    if (actualWallet <= 0) {
        info.textContent = 'Plus de jetons !'
        return;
    }
