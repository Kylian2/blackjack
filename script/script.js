...
const listCout = [
  // contient les prix pour passer à la case suivante
...
function areThereAnyCookies() { 
  /* Cherche l'existence de cookies et si les cookies ont été accepter
  Si oui, cela prolonge leurs durée */
...
function newUser(name, password) {
  // fonction utisée avec la page de connection, NON-UTILISABLE, permet de stocké les
  // donnée le l'utilisateur. La fonction n'est pas terminée puisque nous avons décidé
  // de ne pas utlisé la page de connection 
  user = {
    name: name,
    password: password,
...
function melangerDeck() {
  // range les cartes du deck dans un ordre aléatoire 
...
function reduireAsJoueur() {
  // reduit l'as du joueur à 1 si sa somme depasse 21 et qu'un as est sur son plateau 
...
function reduireAsDealer() {
  // reduit l'as du dealer à 1 si sa somme depasse 21 et qu'un as est sur son plateau 
...
function dealerGoTo17() {
  // fonction qui fait tirer le dealer. Couplé à une boucle while, le force à tirer tant qu'il n'a pas un score > 17
...
function finPartie(joueurSomme, dealerSomme) {
  // fonction utlisé lors de la deuxieme partie de la manche, lorsque le joueur à terminer de jouer
  if (joueurSomme > 21) {
    blocker = false;
    document.cookie = `wallet=${actualWallet};`;
...
function reinitialiser() {
  // Réinistialise la manche, utlisé lorsque la manche est terminée
...
function double() {
  // Fonction appellé lorsque la touche double est préssée, double la mise et tire une carte, fini le tour du joueur
  blocker == true;
  aEteDouble = true;
...
function hit() {
  // Fonction appellé lorsque la touche hit est préssée, tire une carte
  sommeCarteJoueur(distribueCarte());
  nombreCarteJoueurTirer += 1;
  quelleCarteAfficherJoueur(derniereCarteTirer);
  infoSommeJoueur.textContent = joueurSomme;
...
function stand() {
  // Fonction appellé lorsque la touche stand est préssée, permet de finir le tour
  aEteDouble = false;
  quelleCarteAfficherDealer(deuxiemeCarteTirerDealer);
  infoSommeDealer.textContent = newDealerSomme;
  bloquerLesBoutons();
...
window.addEventListener(
  // activé chaque appuie sur une touche du clavier, permet d'augmenter la mise ou de jouer uniquement avec le clavier
  "keydown",
  function (keyPressed) {
    switch (event.code) {
      case "ArrowDown":
...
function isWin() {
  // affiche l'écran de fin de jeu lorsque la partie est terminé
  if (actualCase == 15) {
...
function recommencer() {
  // Sert à remettre le jeu à 0 pour recommencer un nouvelle partie lorsque le jeu est terminé 
